...
- **域名邮箱**：使用自定义域名邮箱（开启 Catch-All 功能）来批量接收注册验证码。如果不知道如何设置域名邮箱，可以参考 Linux.do 的这篇教程。
- **自动化注册**：借助注册机批量产出 Free 账号。例如开源工具 codex-register。
- **导入 CPA**：将批量产出的账号 Token 统一导入 CPA 系统。
